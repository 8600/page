...
window.onload = function() {
  ...
  if (entryDom) {
    ...
    console.error('找不到页面入口!')
      ...
      window.ozzx.script = {"voice":{"data":{"app":null,"loader":null,"container":null,"titleStart":null,"imgArr":["./images/1-title.png"]},"created":function created(){var _this2=this;var loadingTextDom=$('#loadingText')[0];var bodySize=this.methods.calculationScene(this);this.data.loader=new PIXI.loaders.Loader();this.data.loader.add("bgm","audio/bgm.mp3").add("nishuo1","audio/1nishuo.mp3").add("dida","audio/dida.mp3").add(this.data.imgArr).onProgress.add(function(e){loadingTextDom.innerText=e.progress+'%';if(e.progress===100){var titleHandTween=TweenMax.fromTo(oneHand,1.5,{x:bodySize.w*0.44},{x:bodySize.w*0.49,ease:Linear.easeNone}).repeat(-1);titleHandTween.play();var oneButterflyAnimation=TweenMax.fromTo(oneButterfly,0.8,{y:0},{y:-1,ease:Linear.easeNone}).repeat(-1);oneButterflyAnimation.play();_this2.data.app.stage.addChild(one);$('#loadingBox').remove();_this2.data.loader.resources.bgm.data.loop=true;_this2.data.loader.resources.bgm.data.autoplay=true;_this2.data.loader.resources.bgm.data.play();}});this.data.container=new PIXI.Container();this.data.container.interactive=true;var one=new PIXI.Container();one.x=0;one.y=0;var bg1Image=this.methods.createSprite("./images/1.png",{width:bodySize.w,height:bodySize.h,x:0,y:0});var logoImageHeight=bodySize.h/8;var logoImageWidth=logoImageHeight*2.75;var logoImage=this.methods.createSprite("./images/logo.png",{width:logoImageWidth,height:logoImageHeight,x:0,y:bodySize.h-logoImageHeight});var oneTitleHeight=bodySize.h/20;var oneTitleWidth=oneTitleHeight*9.818;var oneTitle=this.methods.createSprite("./images/1-title.png",{width:oneTitleWidth,height:oneTitleHeight,x:bodySize.w/2-oneTitleWidth/2,y:bodySize.h*0.1});var clockHeight=bodySize.h/1.2;var clockWidth=clockHeight*1.7801;var oneClock=this.methods.createSprite("./images/1-clock.png",{width:clockWidth,height:clockHeight,x:bodySize.w/2-clockWidth/2,y:bodySize.h*0.55-clockHeight/2,zIndex:9});var pointHeight=bodySize.h/4;var onePoint=this.methods.createSprite("./images/1-point.png",{width:pointHeight,height:pointHeight,x:bodySize.w/2-clockWidth/2+clockWidth*0.5-2,y:bodySize.h*0.55-clockHeight/2+clockHeight*0.5-2,zIndex:999});onePoint.anchor.x=0.5;onePoint.anchor.y=0.5;var handHeight=bodySize.h/20;var oneHand=this.methods.createSprite("./images/1-hand.png",{width:handHeight,height:handHeight,x:bodySize.w*0.4,y:bodySize.h*0.46-handHeight/2,zIndex:2});var butterflyHeight=bodySize.h/1;var butterflyWidth=bodySize.w;var oneButterfly=this.methods.createSprite("./images/1-butterfly.png",{width:butterflyWidth,height:butterflyHeight,x:0,y:0,zIndex:1});one.addChild(bg1Image,logoImage,oneTitle,oneClock,oneHand,onePoint,oneButterfly);$("#main").on("touchstart",function(e){var touchX=e.originalEvent.targetTouches[0].clientX;var touchY=e.originalEvent.targetTouches[0].clientY;var touchXRange=touchX/bodySize.w;var touchYRange=touchY/bodySize.h;if(touchXRange>0.43&&touchXRange<0.46&&touchYRange>0.42&&touchYRange<0.48){$("#main").on("touchend",function(e){$("#main").off("touchmove");});$("#main").on("touchmove",function(e){var proportion=e.originalEvent.targetTouches[0].clientX/bodySize.w;proportion=(proportion-0.438)/0.06;if(proportion>1)proportion=1;if(proportion<0)proportion=0;onePoint.rotation=0.6*proportion;});}});this.data.loader.load(function(loader){});},"methods":{"createSprite":function createSprite(name,opt){var newSprite=new PIXI.Sprite.from(name);if(opt){for(var key in opt){newSprite[key]=opt[key];}}return newSprite;},"calculationScene":function calculationScene(_this){var ww=$(window).width();var wh=$(window).height();if(false){if(window.orientation!==90&&window.orientation!==-90){window.document.body.classList.add('horizontal');window.document.body.style.width="".concat(wh,"px");window.document.body.style.height="".concat(ww,"px");window.document.body.style.left="calc(50% - ".concat(wh/2,"px)");window.document.body.style.top="calc(50% - ".concat(ww/2,"px)");_this.data.app=new PIXI.Application($(window).height(),$(window).width(),{backgroundColor:'0xffffff'});$('#main').append(_this.data.app.view);_this.data.app.stage.displayList=new PIXI.display.Stage();return{w:wh,h:ww};}}else{if(ww<wh){window.document.body.classList.add('horizontal');window.document.body.style.width="".concat(wh,"px");window.document.body.style.height="".concat(ww,"px");window.document.body.style.left="calc(50% - ".concat(wh/2,"px)");window.document.body.style.top="calc(50% - ".concat(ww/2,"px)");_this.data.app=new PIXI.Application($(window).height(),$(window).width(),{backgroundColor:'0xffffff'});$('#main').append(_this.data.app.view);_this.data.app.stage.displayList=new PIXI.display.Stage();return{w:wh,h:ww};}}_this.data.app=new PIXI.Application($(window).width(),$(window).height(),{backgroundColor:'0xffffff'});$('#main').append(_this.data.app.view);_this.data.app.stage.displayList=new PIXI.display.Stage();return{w:ww,h:wh};}}}}
