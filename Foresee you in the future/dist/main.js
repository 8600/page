...
window.onload = function() {
  ...
  if (entryDom) {
    ...
    console.error('找不到页面入口!')
      ...
      window.ozzx.script = {"voice":{"data":{"app":null,"loader":null,"container":null,"titleContainer":null,"titleStart":null,"imgArr":["./images/1-title.png"]},"created":function created(){var _this2=this;this.methods.calculationScene(this);$('#main').append(this.data.app.view);this.data.app.stage.displayList=new PIXI.display.Stage();var greenGroup=new PIXI.display.Group(1,false);this.data.loader=new PIXI.loaders.Loader();this.data.loader.add("bgm","audio/bgm.mp3").add("nishuo1","audio/1nishuo.mp3").add("dida","audio/dida.mp3").add(this.data.imgArr).onProgress.add(function(e){console.log(e);});this.data.container=new PIXI.Container();this.data.container.interactive=true;var titleContainer=new PIXI.Container();titleContainer.x=0;titleContainer.y=0;var titleStart=this.methods.createSprite("./images/1-title.png",{x:12,y:276});titleContainer.addChild(titleStart);$("#main").on("click",function(){console.log('sd');_this2.data.loader.load(function(loader){loader.resources.bgm.data.loop=true;loader.resources.bgm.data.autoplay=true;loader.resources.bgm.data.play();_this2.data.app.stage.addChild(titleContainer);});});},"methods":{"createSprite":function createSprite(name,opt){var newSprite=new PIXI.Sprite.fromImage(name);if(opt){for(var key in opt){newSprite[key]=opt[key];}}return newSprite;},"calculationScene":function calculationScene(_this){console.log(_this);var ww=$(window).width();var wh=$(window).height();if(false){if(window.orientation!==90&&window.orientation!==-90){window.document.body.classList.add('horizontal');window.document.body.style.width="".concat(wh,"px");window.document.body.style.height="".concat(ww,"px");window.document.body.style.left="calc(50% - ".concat(wh/2,"px)");window.document.body.style.top="calc(50% - ".concat(ww/2,"px)");_this.data.app=new PIXI.Application($(window).height(),$(window).width(),{backgroundColor:'0xffffff'});return;}}else{if(ww<wh){window.document.body.classList.add('horizontal');window.document.body.style.width="".concat(wh,"px");window.document.body.style.height="".concat(ww,"px");window.document.body.style.left="calc(50% - ".concat(wh/2,"px)");window.document.body.style.top="calc(50% - ".concat(ww/2,"px)");_this.data.app=new PIXI.Application($(window).height(),$(window).width(),{backgroundColor:'0xffffff'});return;}}_this.data.app=new PIXI.Application($(window).width(),$(window).height(),{backgroundColor:'0xffffff'});}}}}
