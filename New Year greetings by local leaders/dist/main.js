...
function switchPage (oldUrlParam, newUrlParam) {
  ...
  runPageFunction(newPage, newDom)
      ...
      window.ozzx.script = {"voice":{"data":{"talkTime":0,"clock":null},"created":function created(){var _this=this;document.addEventListener('WeixinJSBridgeReady',function(){_this.domList.autoplayAudio.play();});if(this.data.clock!==null){clearInterval(this.data.clock);this.data.clock=null;}if(this.domList.videoPlay&&this.domList.videoPlay.src&&this.domList.videoPlay.src.indexOf('mp4')!=-1){this.domList.audioPeopleBox.style.display='flex';this.domList.audioPeopleBox.classList.add("video-play");this.domList.speechStateBox.classList.add("video-play");}else{this.domList.audioPeopleBox.style.display='';}this.data.talkTime=0;this.domList.answerBox.style.display='';this.domList.hangUpBox.style.display='';this.domList.bottomBar.style.display='';this.domList.speechStateBox.style.display='';console.log(this.domList.autoplayAudio);},"methods":{"answerSpeech":function answerSpeech(){var _this2=this;if(this.domList.videoPlay&&this.domList.videoPlay.src&&this.domList.videoPlay.src.indexOf('mp4')!=-1){this.domList.videoPlay.style.display='block';this.domList.videoPlay.play();}else if(this.domList.audioPlay&&this.domList.audioPlay.src){this.domList.audioPlay.currentTime=0;this.domList.audioPlay.play();}var talkTime=0;this.domList.answerBox.style.display='none';this.domList.hangUpBox.style.display='flex';this.domList.audioPeopleBox.style.display='none';this.domList.bottomBar.style.display='none';this.domList.speechStateBox.style.display='block';this.data.clock=setInterval(function(){_this2.data.talkTime++;var minute=Math.floor(_this2.data.talkTime/60);if(minute<10)minute='0'+minute;var second=_this2.data.talkTime%60;if(second<10)second='0'+second;_this2.domList.talkTime.innerText=minute+':'+second;},1000);},"stopPlaying":function stopPlaying(){if(this.domList.videoPlay&&this.domList.videoPlay.src&&this.domList.videoPlay.src.indexOf('mp4')!=-1){this.domList.videoPlay.pause();this.domList.videoPlay.style.display='none';this.domList.videoPlay.currentTime=0;window.location.href='#share';}else{this.domList.audioPlay.pause();window.location.href='#share&in=moveToTop&out=moveFromBottom';}},"like":function like(){var rand=Math.floor(Math.random()*100+1);var flows=["flowOne","flowTwo","flowThree"];var colors=["like-1","like-2","like-3","like-4","like-5","like-6"];var timing=(Math.random()*(3.3-1.0)+1.0).toFixed(1);$('<div class="particle part-'+rand+' '+colors[Math.floor(Math.random()*6)]+'" style="font-size:'+Math.floor(Math.random()*(40-22)+22)+'px;"><i class="glyphicon glyphicon-heart">新年快乐</i></div>').appendTo('.ox-voice').css({animation:""+flows[Math.floor(Math.random()*3)]+" "+timing+"s linear"});$('.part-'+rand).show();setTimeout(function(){$('.part-'+rand).remove();},timing*1000-100);}},"template":{"topBar":{"created":function created(){console.log(this);var myDate=new Date();var hours=myDate.getHours();var minutes=myDate.getMinutes();if(hours<10)hours='0'+hours;if(minutes<10)minutes='0'+minutes;this.$el.innerText=hours+':'+minutes;}}}},"share":{"methods":{"returnPage":function returnPage(){window.location.href='#voice&in=moveToBottom&out=moveFromTop';}}}}
